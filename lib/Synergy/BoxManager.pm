...
sub _get_my_ssh_key_file ($self) {
  ...
  unless ($key_file && -r $key_file) {
    $self->handle_log(["Cannot read SSH key for inabox setup (from %s)", $self->ssh_key_id]);
    $self->handle_error(
...
async sub find_and_destroy_droplet ($self, $arg) {
  ...
  my $can_destroy = $force || $droplet->{status} ne 'active';
  ...
  $self->handle_message("Box destroyed: " . $self->box_name_for($username, $tag));
  return;
}
