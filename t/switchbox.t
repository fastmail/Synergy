...
subtest "zero_ok tests" => sub {
  my $box = Test::SwitchBox->new({
    schema => {
      zeroplus => { type => 'str', multi => 1, zero_ok => 1 },
      maybenil => { type => 'str', multi => 1, zero_ok => 1 },
    },
  });

  my $set = $box->switches_ok(
    "/zeroplus",
    {
      zeroplus => [ ],
      maybenil => [ ],
    },
    "multi-value switch that is zero-valued",
  );

  ok(  $set->has_zeroplus, "->has_zeroplus is true (it's empty)");
  ok(! $set->has_maybenil, "->has_maybenil is false (it's absent)");
};
